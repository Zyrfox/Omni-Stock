...
export default async function StoreDetailPage({ params }: PageProps) {
    ...
    const outletId = decodeURIComponent(id);

    // Get all menus for this outlet
    const menus = await db.select().from(masterMenu).where(eq(masterMenu.outlet_id, outletId));
    ...
    const uploadHistory = await db
        .select()
        .from(salesReports)
        .where(eq(salesReports.outlet_id, outletId))
        ...
        <div className="flex flex-col gap-6">
            ...
            <div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">{outletId}</h2>
                        <p className="text-slate-500 dark:text-slate-400">Detail informasi outlet</p>
                    </div>
                </div>
            </div>
